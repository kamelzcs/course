...
mapParser ::
  ...
  -> Parser b
mapParser f (P p) =
  P (\input -> case p input of ErrorResult e -> ErrorResult e
...
bindParser ::
  ...
  -> Parser b
bindParser f (P p) =
  P $ \i -> case p i of Result r a -> parse (f a) r
...
(|||) ::
  ...
  -> Parser a
P p1 ||| P p2 =
  P (\s -> let v = p1 s in if isErrorResult v then p2 s else v)
...
list1 k =
  flbindParser k (\a1 ->
    flbindParser (list k) (\a2 -> valueParser (a1 :. a2)))
...
satisfy ::
  ...
  -> Parser Char
satisfy p =
  bindParser (\c -> if p c then valueParser c else unexpectedCharParser c)
             character
...
sequenceParser ::
  ...
  -> Parser (List a)
sequenceParser =
  foldRight (\pl pr -> flbindParser pl (\l -> flbindParser pr (\r -> valueParser (l :. r))))
            (valueParser Nil)
...
firstNameParser ::
  Parser Chars
firstNameParser =
  flbindParser upper (\c -> flbindParser (list lower) (\s -> valueParser (c :. s)))

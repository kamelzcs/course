...
instance Bind (State s) where
  (=<<) ::
    ...
    -> State s b
  f =<< State k =
    State $ \s -> let (a, s') = k s in runState (f a) s'
...
findM p (h :. t) =
  p h >>= \q -> if q then return $ Full h else findM p t
...
firstRepeat ::
  ...
  -> Optional a
firstRepeat l =
  eval (findM (\a -> State $ S.member a &&& S.insert a) l) S.empty
...
distinct ::
  ...
  -> List a
distinct l =
  eval (filtering (\a -> State $ S.notMember a &&& S.insert a) l) S.empty
...
isHappy =
  contains 1 .
  firstRepeat . produce (toInteger . sum . map (join (*) . digitToInt) . show')

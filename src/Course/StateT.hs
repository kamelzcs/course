...
instance Monad f => Bind (StateT s f) where
  (=<<) ::
    ...
    -> StateT s f b
  f =<< StateT k =
    StateT $ \s -> k s >>= (\(a, t) -> runStateT (f a) t)
...
distinctF ::
  ...
  -> Optional (List a)
distinctF x =
  evalT (filtering (\a -> StateT (\s -> if a > 100 then Empty else pure (a `S.notMember` s, a `S.insert` s))) x) S.empty
...
instance Monad f => Bind (OptionalT f) where
  g =<< OptionalT x =
    OptionalT $ x >>= (\o -> case o of Empty -> pure Empty
